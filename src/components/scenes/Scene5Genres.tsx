...
export function Scene5Genres({ concerts }: Scene5GenresProps) {
  ...
  useEffect(() => {
    ...
    const renderSunburst = () => {
      ...
      const tooltipBg = tooltip.append('rect')
        .attr('rx', 6)
        .attr('ry', 6)
        .attr('fill', '#1f2937') // Dark tooltip bg
        .attr('stroke', '#374151')
        .attr('stroke-width', 1)
      ...
      const tooltipText = tooltip.append('text')
        ...
        .attr('fill', '#f9fafb') // White text

      const tooltipSubtext = tooltip.append('text')
        .attr('font-family', 'Source Sans 3, system-ui, sans-serif')
        .attr('font-size', '12px')
        .attr('fill', '#d1d5db') // Gray text
      ...
      centerText.append('text')
        ...
        .attr('fill', '#f9fafb') // White center count
      ...
      centerText.append('text')
        ...
        .attr('fill', '#d1d5db') // Gray 300 label
        .attr('font-family', 'Source Sans 3, system-ui, sans-serif')
  ...
  useEffect(() => {
    ...
    svg.select('.center-genre')
      ...
      .attr('fill', focusedGenre ? '#d1d5db' : '#9ca3af') // Lighter text for dark mode
  ...
  return (
    <motion.section
      ...
      className="h-screen flex flex-col items-center justify-center relative snap-start snap-always bg-slate-900"
    // style={{ backgroundColor: '#ede9fe' }} // Removed inline style to use bg-slate-900
    >
      {/* Title */}
      <motion.div
      ...
      >
        <h2 className="font-serif text-4xl md:text-7xl text-white mb-2 md:mb-3 tracking-tight">
          The Music
        </h2>
        <p className="font-sans text-base md:text-xl text-gray-300">
      ...
      {focusedNode !== 'All Genres' && (
        <motion.button
          ...
          className="absolute top-24 md:top-32 right-4 md:right-8 z-20 px-4 md:px-6 py-3 bg-slate-800/80 backdrop-blur-sm text-white border border-slate-700 rounded-lg font-sans text-sm font-medium hover:bg-slate-700 transition-all duration-200 shadow-sm min-h-[44px] touchable-no-scale"
        >
          Reset View
        </motion.button>
      )}
